...
void iDraw()
{
    ...
    int ysc;
	ysc=7;
 FILE *pointer1 = fopen("filescr.txt","a");
    //  scanf("%s %d",&na,&ysc);
     fprintf(pointer1,"%s %d\n",na,ysc);
          fclose(pointer1);
    FILE *ptr = fopen("enemy.txt","r");
    fscanf(ptr,"%d\n",&n);
    n++;
    printf("%d\n",n);
    fclose(ptr);
   // *ptr = fopen("taxy,txt","w");
 FILE *pt2 = fopen("enemy.txt","w");
     fprintf(pt2,"%d\n",n);
     fclose(pt2);

FILE *ptr4=fopen("filescr.txt","r");
  for(int i=0;i<n;i++)
  {

 fscanf(pointer1,"%s %d\n",&a1[i].nameofplayer,&a1[i].scoreofninja);

}
  bubblesort(a1);
  FILE *po3=fopen("filescr.txt","w");
for(int i=0;i<n;i++)
{
printf("%s %d\n",a1[i].nameofplayer,a1[i].scoreofninja);
fprintf(po3,"%s %d",a1[i].nameofplayer,a1[i].scoreofninja);
